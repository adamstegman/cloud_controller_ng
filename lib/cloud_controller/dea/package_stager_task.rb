module VCAP::CloudController
  module Dea
    class PackageStagerTask
      ...
      def stage(staging_message, droplet_guid, &completion_callback)
        ...
        @message_bus.publish('staging.stop', app_id: staging_message.guid)
        ...
        # logger.info('staging.begin', app_guid: package.guid)
        staging_result = EM.schedule_sync do |promise|
          ...
          @multi_message_bus_request.on_response(staging_timeout) do |response, error|
            logger.info('staging.first-response', app_guid: staging_message.guid, response: response, error: error)
            handle_first_response(staging_message.guid, response, error, promise)
          end
          ...
          @multi_message_bus_request.on_response(staging_timeout) do |response, error|
            logger.info('staging.second-response', app_guid: staging_message.guid, response: response, error: error)
            handle_second_response(staging_message.guid, response, error)
          end

          package_staging_request = staging_message.staging_request(droplet_guid)
