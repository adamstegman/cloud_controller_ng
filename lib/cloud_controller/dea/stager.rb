module VCAP::CloudController
  module Dea
    class Stager
      ...
      def stage_package(droplet, stack, memory_limit, disk_limit)
        blobstore_url_generator = CloudController::DependencyLocator.instance.blobstore_url_generator
        task = PackageStagerTask.new(@config, @message_bus, @dea_pool, @stager_pool, blobstore_url_generator)

        staging_message = PackageDEAStagingMessage.new(@app, stack, memory_limit, disk_limit, @config, blobstore_url_generator)

        task.stage(staging_message, droplet.guid) do |staging_result|
