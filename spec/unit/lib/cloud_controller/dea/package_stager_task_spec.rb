...
module VCAP::CloudController
  describe Dea::PackageStagerTask do
    ...
    let(:disk_limit) { 321 }
    let(:staging_message) { Dea::PackageDEAStagingMessage.new(package, stack, memory_limit, disk_limit, config_hash, blobstore_url_generator) }
    ...
    context 'when no stager can be found' do
      ...
      it 'should raise an error' do
        expect {
          staging_task.stage(staging_message, task_id)
    ...
    context 'when a stager can be found' do
      it 'should stop other staging tasks' do
        expect(message_bus).to receive(:publish).with('staging.stop', hash_including({ app_id: package.guid }))
        staging_task.stage(staging_message, task_id)
      end
    end
    ...
    describe 'receiving the first response from the stager (the staging setup completion message)' do
      def stage(&blk)
        ...
        response = staging_task.stage(staging_message, task_id, &blk)
        response
      end
      ...
      context 'when staging setup succeeds' do
        context 'when there are available stagers' do
          it 'stops other staging tasks and starts a new one' do
            ...
            expect(message_bus).to receive(:publish).with('staging.my_stager.start', staging_message.staging_request(task_id))
    ...
    describe 'receiving staging completion message' do
      def stage(&blk)
        ...
        staging_task.stage(staging_message, task_id, &blk)
