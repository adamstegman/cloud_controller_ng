...
module VCAP::CloudController
  describe Dea::StagingMessage do
    ...
    describe Dea::PackageDEAStagingMessage do
      ...
      let(:disk_limit) { 321 }
      subject(:staging_message) { Dea::PackageDEAStagingMessage.new(package, stack, memory_limit, disk_limit, config_hash, blobstore_url_generator) }

      its(:stack) { should eq('trusty32') }
      its(:memory_limit) { should eq(1234) }
      its(:disk_limit) { should eq(321) }
      its(:guid) { should eq(package.guid) }
      ...
      describe '#staging_request' do
        it 'includes app guid, task id and download/upload uris' do
          ...
          allow(blobstore_url_generator).to receive(:package_droplet_upload_url).with(task_id).and_return('http://www.droplet.upload.uri')
          request = staging_message.staging_request(task_id)

          expect(request[:app_id]).to eq(package.guid)
